...
public class Snow : Actor, IHaveSprites
{
	...
	public virtual void CollectSprites(List<Sprite> populate)
	{
		...
		for (float z = az; z <= bz; z += area)
		{
			...
			{
				Vec3 point = new() { X = x + Mod(rng.Float(area) + rng.Float(5, 25) * time * Direction.X, area), Y = y + Mod(rng.Float(area) + rng.Float(5, 25) * time * Direction.Y, area), Z = z + Mod(rng.Float(area) + rng.Float(5, 25) * time * Direction.Z, area) };
